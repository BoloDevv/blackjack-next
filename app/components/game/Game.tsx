...
function Game() {
    ...
    return (
        <div className="min-h-screen">
            {gameStarted ? (
                <VisualGame hit={hit} stand={stand} players={players} dealer={dealer} isGameOver={isGameOver}
                            isGameWin={isGameWin} resetGame={resetGameState}/>
            ) : (
                    ...
                    <div className="min-h-screen relative flex items-center justify-center overflow-hidden">
                        ...
                        <div className="relative z-10 text-center px-4">
                            <h1 className="text-6xl md:text-7xl mb-6 text-transparent bg-clip-text drop-shadow-2xl">
                                <span
                                    className={`text-transparent font-bold bg-clip-text bg-gradient-to-r from-main-btn to-yellow-200 ${fontaic.className}`}>Royal Blackjack</span>
                            </h1>
                            <p className="text-xl md:text-2xl mb-8 text-gray-300 max-w-2xl mx-auto">Sinta a adrenalina
                                do 21 na ponta dos seus dedos. </p>
                            <button
                                className="group relative inline-flex items-center gap-2 px-8 py-4 bg-main-btn hover:bg-yellow-400text-black font-bold text-lg rounded-full transition-all duration-300 transform hover:scale-105 hover:shadow-xl cursor-pointer text-black"
                                onClick={() => startGame()}>
